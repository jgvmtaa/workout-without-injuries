...
/**
 * Immutable state for the training preferences screen (README §4.3, §20).
 *
 ...
 *
 * [derivedSplit] is output, not input. README §4.3 permits the MVP to choose the split
...
data class PreferencesUiState(
    ...
    /** Frequencies offered, in order (README §4.3). */
    val dayOptions: List<Int> = TrainingOptions.DAYS_PER_WEEK

    /** Session lengths offered, in order (README §4.3). */
     ...
     * because it is always available (README §25). Neither is `CARDIO_MACHINE`: the MVP
     ...
     * "bodyweight only", which the domain guarantees is never empty (README §25).
     */
...
/** Everything the preferences screen can do (README §20). */
