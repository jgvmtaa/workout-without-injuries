...
/**
 * The numeric choices the preferences screen offers (spec §4.3).
 *
...
object TrainingOptions {
    ...
    /** Training days per week the MVP supports (spec §4.3). */
    val DAYS_PER_WEEK: List<Int> = listOf(2, 3, 4, 5)

    /** Approximate session lengths, in minutes (spec §4.3). */
     ...
     * possible conditioning work.
     *
     * [Equipment.BODYWEIGHT] is absent too, for the opposite reason — it is always
     * available and cannot be deselected, so it is never a choice (spec §25).
     */
